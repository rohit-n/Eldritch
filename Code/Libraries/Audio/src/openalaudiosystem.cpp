...
ISoundInstance* OpenALAudioSystem::Play( const SimpleString& DefinitionName, const Vector& Location )
{
...
/*virtual*/ void OpenALAudioSystem::ConditionalApplyReverb( ISoundInstance* const pSoundInstance ) const
{
	...
	for( uint ReverbCategoryIndex = 0; ReverbCategoryIndex < m_ReverbCategories.Size(); ++ReverbCategoryIndex )
	{
		...
		{
			OpenALSoundInstance* const pOALInstance = static_cast<OpenALSoundInstance*>( pSoundInstance );
...
void OpenALAudioSystem::SetReverbParams( const SimpleString& DefinitionName ) const
{
