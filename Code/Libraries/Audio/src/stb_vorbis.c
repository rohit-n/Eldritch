...
typedef struct
{
   ...
   unsigned char *codeword_lengths;
   float  minimum_value;
   float  delta_value;
   unsigned char  value_bits;
   unsigned char  lookup_type;
   unsigned char  sequence_p;
   unsigned char  sparse;
   unsigned int lookup_values;
   codetype *multiplicands;
   unsigned int *codewords;
   #ifdef STB_VORBIS_FAST_HUFFMAN_SHORT
    signed short  fast_huffman[FAST_HUFFMAN_TABLE_SIZE];
   #else
    signed int  fast_huffman[FAST_HUFFMAN_TABLE_SIZE];
   #endif
...
typedef struct
{
   unsigned char order;
   unsigned short rate;
   unsigned short bark_map_size;
   unsigned char amplitude_bits;
   unsigned char amplitude_offset;
   unsigned char number_of_books;
   unsigned char book_list[16]; // varies
} Floor0;

typedef struct
{
   unsigned char partitions;
   unsigned char partition_class_list[32]; // varies
   unsigned char class_dimensions[16]; // varies
   unsigned char class_subclasses[16]; // varies
   unsigned char class_masterbooks[16]; // varies
   signed short subclass_books[16][8]; // varies
   unsigned short Xlist[31*8+2]; // varies
   unsigned char sorted_order[31*8+2];
   unsigned char neighbors[31*8+2][2];
   unsigned char floor1_multiplier;
...
typedef struct
{
   ...
   unsigned char classbook;
   unsigned char **classdata;
   signed short (*residue_books)[8];
} Residue;
...
typedef struct
{
   unsigned short coupling_steps;
   MappingChannel *chan;
   unsigned char  submaps;
   unsigned char  submap_floor[15]; // varies
   unsigned char  submap_residue[15]; // varies
} Mapping;
...
typedef struct
{
   unsigned int  goal_crc;    // expected crc if match
   int     bytes_left;  // bytes left in packet
   unsigned int  crc_so_far;  // running crc
   int     bytes_done;  // bytes processed in _current_ chunk
   unsigned int  sample_loc;  // granule pos encoded in page
} CRCscan;
...
struct stb_vorbis
{
  ...
  // header info
   ...
   Codebook *codebooks;
   int floor_count;
   unsigned short floor_types[64]; // varies
   Floor01 *floor_config;
   int residue_count;
   unsigned short residue_types[64]; // varies
  ...
  // decode buffer
   ...
   signed short *finalY[STB_VORBIS_MAX_CHANNELS];
   #else
   float *floor_buffers[STB_VORBIS_MAX_CHANNELS];
   #endif

   unsigned int current_loc; // sample location of next frame to decode
  ...
  // per-blocksize precomputed data
   ...
   float *A[2],*B[2],*C[2];
   float *window[2];
   unsigned short *bit_reverse[2];

  // current page/packet/segment streaming info
   unsigned int serial; // stream serial number for verification
   int last_page;
   int segment_count;
   unsigned char segments[255];
   unsigned char page_flag;
   unsigned char bytes_in_seg;
   unsigned char first_decode;
   int next_seg;
...
static __forceinline unsigned int crc32_update(unsigned int crc, unsigned char byte)
{
...
static float float32_unpack(unsigned int x)
{
   // from the specification
   unsigned int mantissa = x & 0x1fffff;
   unsigned int sign = x & 0x80000000;
   unsigned int exp = (x & 0x7fe00000) >> 21;
...
static void add_entry(Codebook *c, unsigned int huff_code, int symbol, int count, int len, unsigned int *values)
{
   if (!c->sparse) {
...
static int compute_codewords(Codebook *c, unsigned char *len, int n, unsigned int *values)
{
   int i,k,m=0;
   unsigned int available[32];
...
static void compute_accelerated_huffman(Codebook *c)
{
   ...
   for (i=0; i < len; ++i) {
      if (c->codeword_lengths[i] <= STB_VORBIS_FAST_HUFFMAN_LENGTH) {
         unsigned int z = c->sparse ? bit_reverse(c->sorted_codewords[i]) : c->codewords[i];
...
static int include_in_sort(Codebook *c, unsigned char len)
{
...
static void compute_sorted_huffman(Codebook *c, unsigned char *lengths, unsigned int *values)
{
   int i, len;
   ...
   for (i=0; i < len; ++i) {
      ...
      if (include_in_sort(c,huff_len)) {
         unsigned int code = bit_reverse(c->codewords[i]);
...
{
   static unsigned char vorbis[6] = { 'v', 'o', 'r', 'b', 'i', 's' };
...
static void compute_bitreverse(int n, unsigned short *rev)
{
...
static unsigned char get8(vorb *z)
{
   if (USE_MEMORY(z)) {
...
static unsigned int get32(vorb *f)
{
   unsigned int x;
   x = get8(f);
   x += get8(f) << 8;
...
static int getn(vorb *z, unsigned char *data, int n)
{
   if (USE_MEMORY(z)) {
...
static unsigned char ogg_page_header[4] = { 0x4f, 0x67, 0x67, 0x53 };
...
static unsigned int get_bits(vorb *f, int n)
{
   unsigned int z;
...
static signed int get_bits_signed(vorb *f, int n)
{
   unsigned int z = get_bits(f, n);
   if (z & (1 << (n-1)))
      z += ~((1 << n) - 1);
   return (int32) z;
}
...
static int codebook_decode_scalar_raw(vorb *f, Codebook *c)
{
   ...
   if (c->entries > 8 ? c->sorted_codewords!=NULL : !c->codewords) {
      // binary search
      unsigned int code = bit_reverse(f->acc);
...
static void decode_residue(vorb *f, float *residue_buffers[], int ch, int n, int rn, unsigned char *do_not_decode)
{
   int i,j,pass;
   ...
   unsigned char ***part_classdata = (uint8 ***) temp_block_array(f,f->channels, part_read * sizeof(**part_classdata));
   #else
...
static void inverse_mdct(float *buffer, int n, vorb *f, int blocktype)
{
   ...
   {
      unsigned short *bitrev = f->bit_reverse[blocktype];
...
#ifndef STB_VORBIS_NO_DEFER_FLOOR
typedef signed short YTYPE;
#else
typedef int YTYPE;
#endif
static int do_floor(vorb *f, Mapping *map, int i, int n, float *target, YTYPE *finalY, unsigned char *step2_flag)
{
   int n2 = n >> 1;
...
// RESIDUE DECODE
   for (i=0; i < map->submaps; ++i) {
      ...
      int r,t;
      unsigned char do_not_decode[256];
      int ch = 0;
...
// INVERSE MDCT
   ...
   if (f->last_seg_which == f->end_seg_with_known_loc) {
      ...
      if (f->current_loc_valid && (f->page_flag & PAGEFLAG_last_page)) {
         unsigned int current_end = f->known_loc_for_packet - (n-right_end);
...
static int start_decoder(vorb *f)
{
   ...
   for (i=0; i < f->codebook_count; ++i) {
      ...
      if (!c->sparse) {
         c->codewords = (unsigned int *) setup_malloc(f, sizeof(c->codewords[0]) * c->entries);
         if (!c->codewords)                  return error(f, VORBIS_outofmem);
      } else {
         unsigned int size;
         if (c->sorted_entries) {
            c->codeword_lengths = (unsigned char *) setup_malloc(f, c->sorted_entries);
            if (!c->codeword_lengths)           return error(f, VORBIS_outofmem);
            c->codewords = (unsigned int *) setup_temp_malloc(f, sizeof(*c->codewords) * c->sorted_entries);
            if (!c->codewords)                  return error(f, VORBIS_outofmem);
            values = (unsigned int *) setup_temp_malloc(f, sizeof(*values) * c->sorted_entries);
      ...
      if (c->sorted_entries) {
         ...
         c->sorted_codewords = (unsigned int *) setup_malloc(f, sizeof(*c->sorted_codewords) * (c->sorted_entries+1));
   ...
   x = get_bits(f, 6) + 1;
   for (i=0; i < x; ++i) {
      unsigned int z = get_bits(f, 16);
      if (z != 0) return error(f, VORBIS_invalid_setup);
   }

   // Floors
   f->floor_count = get_bits(f, 6)+1;
   f->floor_config = (Floor01 *)  setup_malloc(f, f->floor_count * sizeof(*f->floor_config));
   ...
   for (i=0; i < f->residue_count; ++i) {
      unsigned char residue_cascade[64];
      ...
      for (j=0; j < r->classifications; ++j) {
         ...
         unsigned char low_bits=get_bits(f,3);
         if (get_bits(f,1))
   ...
   // 1.
   {
      unsigned int imdct_mem = (f->blocksize_1 * sizeof(float) >> 1);
      unsigned int classify_mem;
...
static int vorbis_search_for_page_pushdata(vorb *f, unsigned char *data, int data_len)
{
   int i,n;
...
static unsigned int vorbis_find_page(stb_vorbis *f, unsigned int *end, unsigned int *last)
{
   for(;;) {
      int n;
      if (f->eof) return 0;
...
{
   unsigned char header[27], lacing[255];
   unsigned char packet_type[255];
...
static int vorbis_seek_frame_from_page(stb_vorbis *f, unsigned int page_start, unsigned int first_sample, unsigned int target_sample, int fine)
{
...
static int vorbis_seek_base(stb_vorbis *f, unsigned int sample_number, int fine)
{
   ...
   if (f->p_last.page_start == 0) {
      unsigned int z = stb_vorbis_stream_length_in_samples(f);
   ...
   if (sample_number < f->p_first.last_decoded_sample) {
      ...
      while (p[0].page_end < p[1].page_start) {
         ...
         if (attempts >= 4) {
            unsigned int probe2 = start_offset + ((end_offset - start_offset) >> 1);
            if (attempts >= 8)
               probe = probe2;
